# Local proposed modification: widen float16 accumulation through bias addition.
...
def _conv_implementation(
    ...
    # Keep float16 accumulation wide through bias addition to avoid early overflow.
    if X.dtype == np.float16:
        mul = np.matmul(w_reshaped, c2, dtype=np.float32)
    else:
        mul = w_reshaped @ c2
    ...
    return mul.astype(X.dtype, copy=False) if X.dtype == np.float16 else mul
